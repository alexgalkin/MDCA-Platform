        ...
        struct PathParser {
          ...
          public:
            ...
            void increment() noexcept {
                ...
                if (Start == End) return makeState(PS_AtEnd);
                switch (State) {
                case PS_BeforeBegin: {
                    ...
                    if (TkEnd)
                        ...
                        return makeState(
                                PS_InFilenames, Start, consumeName(Start, End));
                }
                case PS_InRootDir:
                    return makeState(
                            PS_InFilenames, Start, consumeName(Start, End));
            ...
            void decrement() noexcept {
                ...
                case PS_AtEnd: {
                    ...
                    if (PosPtr SepEnd = consumeSeparator(RStart, REnd)) {
                        if (SepEnd == REnd)
                            return makeState(
                                    PS_InRootDir, Path.data(), RStart + 1);
                        return makeState(
                                PS_InTrailingSep, SepEnd + 1, RStart + 1);
                    } else {
                        PosPtr TkStart = consumeName(RStart, REnd);
                        return makeState(
                                PS_InFilenames, TkStart + 1, RStart + 1);
                    }
                }
                case PS_InTrailingSep:
                    return makeState(
                            PS_InFilenames, consumeName(RStart, REnd) + 1,
                            RStart + 1);
                case PS_InFilenames: {
          ...
          private:
            void makeState(
                    ParserState NewState, PosPtr Start, PosPtr End) noexcept {
                State = NewState;
            ...
            /// \brief Return a pointer to the first character after the
            /// currently
            ///   lexed element.
    ...
    namespace {
        ...
        struct FileDescriptor {
            ...
            static FileDescriptor
                    create(const path *p, error_code &ec, Args... args) {
                ec.clear();
                int fd;
            ...
            template<class... Args>
            static FileDescriptor create_with_status(
                    const path *p, error_code &ec, Args... args) {
        ...
        file_status create_file_status(
                error_code &m_ec,
                path const &p,
            ...
            if (m_ec && (m_ec.value() == ENOENT || m_ec.value() == ENOTDIR)) {
                ...
                err.report(
                        m_ec,
                        "failed to determine attributes for the specified "
                        "path");
        ...
        file_status
                posix_lstat(path const &p, StatT &path_stat, error_code *ec) {
            error_code m_ec;
        ...
        bool posix_ftruncate(
                const FileDescriptor &fd, size_t to_size, error_code &ec) {
        ...
        bool posix_fchmod(
                const FileDescriptor &fd, const StatT &st, error_code &ec) {
        ...
        file_status FileDescriptor::refresh_status(error_code &ec) {
            ...
            m_stat = {};
            error_code m_ec;
            if (::fstat(fd, &m_stat) == -1) m_ec = capture_errno();
    ...
    __storage_->__what_ = [&]() -> string {
        ...
        case 1:
            return detail::format_string(
                    "filesystem error: %s [%s]", derived_what, p1);
        case 2:
            return detail::format_string(
                    "filesystem error: %s [%s] [%s]", derived_what, p1, p2);
        }
    }();
}
...
void __copy(
        const path &from, const path &to, copy_options options, error_code *ec) {
    ...
    if (is_symlink(f)) {
        ...
        return err.report(errc::is_a_directory);
            ...
            is_directory(f)
            && (bool(copy_options::recursive & options)
        ...
        for (; it != directory_iterator(); it.increment(m_ec2)) {
            if (m_ec2) { return err.report(m_ec2); }
...
namespace detail {
    namespace {
        ...
        bool copy_file_impl_sendfile(
            ...
            do {
                ssize_t res;
                if ((res = ::sendfile(write_fd.fd, read_fd.fd, nullptr, count))
                    == -1) {
                    ec = capture_errno();
                    return false;
                }
        ...
        bool copy_file_impl_copyfile(
            ...
            CopyFileState cfs;
            if (fcopyfile(read_fd.fd, write_fd.fd, cfs.state, COPYFILE_DATA)
                < 0) {
                ec = capture_errno();
                return false;
            }
...
#endif
        // Note: This function isn't guarded by ifdef's even though it may be
        // unused in order to assure it still compiles.
        __attribute__((unused)) bool copy_file_impl_default(
                FileDescriptor &read_fd,
                FileDescriptor &write_fd,
                error_code &ec) {
            ifstream in;
        ...
        bool copy_file_impl(
                FileDescriptor &from, FileDescriptor &to, error_code &ec) {
...
void __create_directory_symlink(
        path const &from, path const &to, error_code *ec) {
...
bool __equivalent(const path &p1, const path &p2, error_code *ec) {
    ...
    if (!exists(s1)) return err.report(errc::not_supported);
    auto s2 = detail::posix_stat(p2.native(), st2, &ec2);
    if (!exists(s2)) return err.report(errc::not_supported);
...
static file_time_type __extract_last_write_time(
        const path &p, const StatT &st, error_code *ec) {
...
namespace {
    uintmax_t remove_all_impl(path const &p, error_code &ec) {
        ...
        uintmax_t count = 1;
        if (is_directory(st)) {
            for (directory_iterator it(p, ec);
                 !ec && it != directory_iterator(); it.increment(ec)) {
...
uintmax_t __remove_all(const path &p, error_code *ec) {
    ...
    if (mec) {
        if (mec == errc::no_such_file_or_directory) return 0;
...
path __temp_directory_path(error_code *ec) {
    ...
    if (!exists(st) || !is_directory(st))
        return err.report(
                errc::not_a_directory, "path \"%s\" is not a directory", p);
    return p;
}
path __weakly_canonical(const path &p, error_code *ec) {
    ErrorHandler<path> err("weakly_canonical", ec, &p);
    if (p.empty()) return __canonical("", ec);
    path result;
    path tmp;
    ...
    if (PP.State == PathParser::PS_BeforeBegin) result = __canonical("", ec);
    if (ec) ec->clear();
    if (DNEParts.empty()) return result;
...
path &path::replace_extension(path const &replacement) {
    path p = extension();
    if (not p.empty()) { __pn_.erase(__pn_.size() - p.native().size()); }
    if (!replacement.empty()) {
        if (replacement.native()[0] != '.') { __pn_ += "."; }
...
string_view_t path::__root_name() const {
    ...
    if (PP.State == PathParser::PS_InRootName) return *PP;
    return {};
}
string_view_t path::__root_directory() const {
    auto PP = PathParser::CreateBegin(__pn_);
    if (PP.State == PathParser::PS_InRootName) ++PP;
    if (PP.State == PathParser::PS_InRootDir) return *PP;
    return {};
}
...
static bool ConsumeRootDir(PathParser *PP) {
    while (PP->State <= PathParser::PS_InRootDir) ++(*PP);
    return PP->State == PathParser::PS_AtEnd;
}
string_view_t path::__relative_path() const {
    auto PP = PathParser::CreateBegin(__pn_);
    if (ConsumeRootDir(&PP)) return {};
...
string_view_t path::__parent_path() const {
    ...
    {
        auto PP = PathParser::CreateBegin(__pn_);
        if (ConsumeRootDir(&PP)) return __pn_;
    }
    ...
    // a string representing that path
    {
        ...
        --PP;
        if (PP.RawEntry.data() == __pn_.data()) return {};
        --PP;
        return createView(__pn_.data(), &PP.RawEntry.back());
    }
}
...
static PathPartKind ClassifyPathPart(string_view_t Part) {
    if (Part.empty()) return PK_TrailingSep;
    if (Part == ".") return PK_Dot;
    if (Part == "..") return PK_DotDot;
    if (Part == "/") return PK_RootSep;
    return PK_Filename;
}
path path::lexically_normal() const {
    if (__pn_.empty()) return *this;
    ...
    bool NeedTrailingSep =
            MaybeNeedTrailingSep && LastPartKind() == PK_Filename;
    path Result;
    Result.__pn_.reserve(Parts.size() + NewPathSize + NeedTrailingSep);
    for (auto &PK : Parts) Result /= PK.first;
    if (NeedTrailingSep) Result /= "";
    return Result;
}
...
path path::lexically_relative(const path &base) const {
    { // perform root-name/root-directory mismatch checks
        ...
        if (CheckIterMismatchAtBase()) return {};
    }
...
int path::__compare(string_view_t __s) const {
    ...
    if (PP.State == PP2.State && !PP) return 0;
    if (!PP) return -1;
    return 1;
}
////////////////////////////////////////////////////////////////////////////
...
error_code directory_entry::__do_refresh() noexcept {
    ...
    if (_VSTD_FS::exists(st)) {
        ...
        // Attempt to extract the mtime, and fail if it's not representable
        // using file_time_type. For now we ignore the error, as we'll report it
