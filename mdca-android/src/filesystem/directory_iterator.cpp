...
namespace detail {
    namespace {
        ...
        static file_type get_file_type(DirEntT *ent, int) {
            ...
            // underlying filesystem doesn't support d_type. Report is as 'none'
            // so we correctly set the cache to empty.
    ...
    bool advance(error_code &ec) {
        while (::FindNextFile(__stream_, &__data_)) {
            ...
            __entry_.__assign_iter_entry(
                    ...
                    directory_entry::__create_iter_result(
                            get_file_type(__data)));
            return true;
        }
        ec = error_code(::GetLastError(), generic_category());
        close();
...
directory_iterator::directory_iterator(
        const path &p, error_code *ec, directory_options opts) {
    ErrorHandler<void> err(
            "directory_iterator::directory_iterator(...)", ec, &p);
    error_code m_ec;
...
directory_iterator &directory_iterator::__increment(error_code *ec) {
    ...
    if (!__imp_->advance(m_ec)) {
        ...
        __imp_.reset();
        if (m_ec) err.report(m_ec, "at root \"%s\"", root);
    }
    return *this;
}
...
recursive_directory_iterator &
    ...
    if (recursion_pending()) {
        if (__try_recursion(ec) || (ec && *ec)) return *this;
    }
    __rec_ = true;
    __advance(ec);
    return *this;
...
bool recursive_directory_iterator::__try_recursion(error_code *ec) {
    ...
    if (m_ec) {
        const bool allow_eacess = bool(
                __imp_->__options_ & directory_options::skip_permission_denied);
